            ...
            if (targetPropertiesNames.Count > 0)
            {
                ...
                for (int i = targetPropertiesNames.Count - 1; i >= 0; i--)
                {
                    ...
                    for (int j = propertyRelativeArrray.arraySize - 1; j >= 0; j--)
                    {
                        ...
                        if (!UnityEventDrawer.IsPersistantListenerValid(dummyEvent, propertyMethodName.stringValue, propertyTarget.objectReferenceValue,
                            (PersistentListenerMode) arrayElementAtIndex.FindPropertyRelative("m_Mode").enumValueIndex, argumentAssemblyType))
                        {
                            if (printError)
                            {
