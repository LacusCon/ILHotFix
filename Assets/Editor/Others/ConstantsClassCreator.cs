...
public static class ConstantsClassCreator
{
    ...
    public static void Create<T>(string className, string classInfo, string directoryPath, Dictionary<string, T> valueDict)
    {
        ...
        //ディクショナリーをソートしたものに
        var sortDict = new SortedDictionary<string, T>(valueDict);
        ...
        foreach (var key in keyArray)
        {
            ...
            //イコールが並ぶ用に空白を調整する
            var EqualStr = String.Format("{0, " + (keyLengthMax - key.Length).ToString() + "}", "=");
        ...
        //書き出し、ファイル名はクラス名.cs
        var exportPath = directoryPath + className + ".cs";
    ...
    private static string SetDelimiterBeforeUppercase(string str)
    {
        ...
        for (var strNo = 0; strNo < str.Length; strNo++)
        {
            var isSetDelimiter = true;
